...
?>
  <?php
      ...
      $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido    where fecha >='$enero_inicio' and fecha <='$enero_fin' ")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
      $cantidad_enero=$row['monto']+$cantidad_enero;
      }
...
$query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido   where fecha >='$febrero_inicio' and fecha <='$febrero_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
     $cantidad_febrero=$row['monto']+$cantidad_febrero;
      }

      $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido   where fecha >='$marzo_inicio' and fecha <='$marzo_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
             $cantidad_marzo=$row['monto']+$cantidad_marzo;

      }

            $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido  where fecha >='$abril_inicio' and fecha <='$abril_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
            $cantidad_abril=$row['monto']+$cantidad_abril;
      $cantidad_abril++;
      }
     
           $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido  where fecha >='$mayo_inicio' and fecha <='$mayo_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
            $cantidad_mayo=$row['monto']+$cantidad_mayo;

      }

          $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido   where fecha >='$junio_inicio' and fecha <='$junio_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
                 $cantidad_junio=$row['monto']+$cantidad_junio;

      }

         $query=mysqli_query($con,"select * from ventas_servicios AS v INNER JOIN detalles_ventas_servicios AS d
      ON v.num_pedido = d.id_pedido  where fecha >='$julio_inicio' and fecha <='$juio_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
//        $var = (float) $row['monto'];
       $cantidad_julio= number_format($row['monto']+$cantidad_julio,2);


      }

        $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido  where fecha >='$agosto_inicio' and fecha <='$agosto_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
        $cantidad_agosto=$row['monto']+$cantidad_agosto;
 
      }
              $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido   where fecha >='$setiembre_inicio' and fecha <='$setiembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
           $cantidad_setiembre=$row['monto']+$cantidad_setiembre;

      }

             $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido  where fecha >='$octubre_inicio' and fecha <='$octubre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
            $cantidad_octubre=$row['monto']+$cantidad_octubre;
 
      }

            $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido  where fecha >='$noviembre_inicio' and fecha <='$noviembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
         $cantidad_noviembre=$row['monto']+$cantidad_noviembre;

      }

           $query=mysqli_query($con,"select * from ventas_servicios AS p INNER JOIN detalles_ventas_servicios AS z
      ON p.num_pedido = z.id_pedido  where fecha >='$diciembre_inicio' and fecha <='$diciembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
         $cantidad_diciembre=$row['monto']+$cantidad_diciembre;
...
$query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
     ...
     $cantidad_febrero=$row['precio_venta']+$cantidad_febrero;
      ...
      while($row=mysqli_fetch_array($query)){
         $cantidad_julio_productos=($row['precio_venta']*$row['cantidad'])+$cantidad_julio_productos;
      ...
      $cantidad_marzo_total=0;
        $cantidad_abril_total=0;
          $cantidad_mayo_total=0;
            ...
            $cantidad_enero_total=$cantidad_enero+$cantidad_enero_productos;
             $cantidad_febrero_total=$cantidad_febrero+$cantidad_febrero_productos; 
                          $cantidad_marzo_total=$cantidad_marzo+$cantidad_marzo_productos;
                                                    $cantidad_abril_total=$cantidad_abril+$cantidad_abril_productos;


          $cantidad_mayo_total=$cantidad_mayo+$cantidad_mayo_productos;
          $cantidad_junio_total=$cantidad_junio+$cantidad_junio_productos;
          $cantidad_julio_total=$cantidad_julio+$cantidad_julio_productos;

          $cantidad_agosto_total=$cantidad_agosto+$cantidad_agosto_productos;
          $cantidad_setiembre_total=$cantidad_setiembre+$cantidad_setiembre_productos;
          $cantidad_octubre_total=$cantidad_octubre+$cantidad_octubre_productos;

          $cantidad_noviembre_total=$cantidad_noviembre+$cantidad_noviembre_productos;
          $cantidad_diciembre_total=$cantidad_diciembre+$cantidad_diciembre_productos;          
  ?>
